...
interface AuthRepository {
    ...
    // Lấy UID người dùng hiện tại một cách đồng bộ
    fun getCurrentUserUid(): String

    // Lấy Email người dùng hiện tại một cách đồng bộ
    ...
    suspend fun updateNotificationSettings(push: Boolean, booking: Boolean, promo: Boolean, system: Boolean): Resource<Unit>

    // Send password reset email
    suspend fun sendPasswordResetEmail(email: String): Resource<Unit>

    // Get PIN code flow (returns empty string if not set)
    fun getPinCode(): Flow<String>

    // Save PIN code
    suspend fun savePinCode(pin: String)

    // Clear/Disable PIN code
    suspend fun clearPinCode()

    fun getPinFailedAttempts(): Flow<Int>
    suspend fun savePinFailedAttempts(attempts: Int)

    fun getPinLockoutTime(): Flow<Long>
    suspend fun savePinLockoutTime(timeInMillis: Long)

    // Login with Google Token
    suspend fun loginWithGoogle(idToken: String): Resource<String>
}
